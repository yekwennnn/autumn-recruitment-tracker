...
def cmd_record(args):
    ...
    for item in evaluated:
        ...
        if item.get("fetched"):
            ...
            # JD 里抓到的截止日期覆盖发现层的粗略值；抓不到就保留原值。
            if item.get("deadline"):
                rec["deadline"] = str(item["deadline"])[:10]
                try:
                    if date.fromisoformat(rec["deadline"]) < date.today():
                        rec["expired"] = True
                except ValueError:
                    del rec["deadline"]
            if int(item["score"]) >= min_score_for_action and rec.get("lane"):
                entry = yields.setdefault(rec["lane"], {})
                entry["hit"] = int(entry.get("hit", 0)) + 1
            scored += 1
        else:
