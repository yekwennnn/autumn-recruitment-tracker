...
存在档案的 `custom_answers` 里。遇到开放题，**按这个顺序找料，别一上来就问用户**：

**素材库（`resumes/experience-bank.json`）→ `custom_answers` → 现场问**

素材库里的 STAR 故事本来就是为"讲一件事"准备的，改写成开放题答案比从头问快得多，
而且都是用户确认过的真事。挖掘方法见 `references/experience-mining.md`。

然后：

1. **先在 `custom_answers` 里做语义匹配**（不是字符串精确匹配——「谈谈你的职业规划」和「请描述你未来三年的发展计划」是同一道题）。
2. 命中 → 把已有答案展示给用户，问：直接用 / 按这家公司微调 / 重写。微调后的版本存进该条的 `company_variants`，通用版保持不动。
3. 没命中 → 先看素材库有没有能用的故事，有就改写；都没有才基于简历和用户口述草拟。用户确认后写入 `custom_answers`（带上 `tags` 和 `created_at`）；如果过程中问出了新的经历细节，**同时往素材库对应经历里存一份**。
