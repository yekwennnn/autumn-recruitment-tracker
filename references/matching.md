...
本文档是深评管线（SKILL.md 第 8 步）和"单个 JD 现评"（简历工坊）的权威规则。
...
python3 $SKILL_DIR/scripts/match_state.py pending \
  ...
  --output $SKILL_DIR/state/tmp/pending.json
# ...抓取 + 评分，写 $SKILL_DIR/state/tmp/evaluated.json...
python3 $SKILL_DIR/scripts/match_state.py record \
  --state $SKILL_DIR/state/seen_postings.json --config $SKILL_DIR/config.json \
  --input $SKILL_DIR/state/tmp/evaluated.json
...
**模型**：抓 JD 是取文本，不是判断，一律用 `resume.jd_fetch_agent_model`（默认 `haiku`）发起。环境的 Agent 工具不支持指定模型就忽略这一项。评分仍然由 orchestrator 亲自做——见下一节，这条不因为省钱而松动。
...
打分规则：
- 证据来源**只有两处**：`resumes/profile.json`（含 0.2 分析产出的 `strengths`/`gaps`）+ `state/match_insights.json` 的 `confirmed_facts`（先跑 `insights.py show` 拿到）。
- **JD 硬性要求在两处都无证据 → 该项按缺失计，禁止脑补"应该会"**——与改简历的实事求是红线同源。
- 每条输出必含：`score`（整数）、`pros`（一句 ≤40 字优势）、`gaps`（一句 ≤40 字差距）、`jd_keywords`；抓到了就带上 `deadline`。
- **评分不下放给子 agent**，任何成本压力下都不例外。子 agent 只负责取文本。
...
"<sha1>": {
  ...
  "lane": "wechat-launch",
  "deadline": "2026-08-15", "expired": false,
...
`match` 字段只由 `match_state.py` 写（dedupe.py 不碰它）；`lane`/`deadline`/`expired` 由 dedupe.py 写入、`match_state.py record` 用 JD 抓到的值更新 `deadline`。`expired` 为 true 的岗位不会再进深评队列（评了也投不了），也不会出现在日报的新增列表里。

state 顶层字段：`wechat_rotation_cursor`（公众号轮询游标）、`last_full_sweep`（上次全量扫描日期）、`source_yield`（各车道产出统计）——这三个都由 `discovery.py` 读写，orchestrator 不要手改；`_last_run_lane_new` 是 dedupe.py 留给 `discovery.py commit` 的临时交接字段，commit 后自动消失。

### state/tmp/evaluated.json（orchestrator 评分后写）

```json
[
  { "id": "<sha1>", "fetched": true, "jd_url": "...", "jd_summary": "≤150字", "jd_keywords": ["Python"], "deadline": "2026-08-15", "score": 84, "pros": "一句优势", "gaps": "一句差距", "resume_version": "20260724-base" },
